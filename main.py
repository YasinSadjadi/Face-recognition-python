...
class Main:
    ...
    def training(self):
        tts = self.tts.tts("if you want to train, enter your name, else press enter")
        sd.play(tts, samplerate=Main.SAMPLE_RATE)
        ...
        tts = self.tts.tts("press 't' to train, press 'Esc' to exit")
        sd.play(tts, samplerate=Main.SAMPLE_RATE)
        sd.wait()
    ...
    def recognizing(self):
        tts = self.tts.tts("if you want training press 't', and if not, press 'Esc' to exit")
        sd.play(tts, samplerate=Main.SAMPLE_RATE)
        # sd.wait()
